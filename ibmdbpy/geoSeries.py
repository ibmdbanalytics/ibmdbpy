...
#==============================================================================
    ...
    def _binary_operation_handler(self, ida2, db2gse_function,
                                ...
                                additional_args = None):
        """
        In progress. Fix how to get the indexer of the involved idas
        for the query for creating the view.
        ...
        # the DB2GSE function
        column1_for_db2gse = ida1.internal_state.columndict[self.column]
        if column1_for_db2gse[0] == '\"' and column1_for_db2gse[-1] == '\"':
                column1_for_db2gse = column1_for_db2gse[1:-1]
        column2_for_db2gse = ida2.internal_state.columndict[self.column]
        ...
        from_clause = (
            'FROM ' +
            '(' + ida1.internal_state.get_state() + ') AS IDA1 '+
            ' CROSS JOIN ' +
            '(' + ida2.internal_state.get_state() + ') AS IDA2 '
        )
        ...
        idageodf = ibmdbpy.IdaGeoDataFrame(self._idadb, viewname)
        return idageodf 