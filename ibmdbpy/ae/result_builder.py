...
def build_result(output_table, merge_output, db, df, output_signature, table_name, query, id):
    ...
    if output_table and merge_output is True:
        ...
        query =  " create table "+output_table+" as select  " + columns_str + " , base.*  from  " + output_table_tmp + " as link INNER JOIN  " + table_name + " as base on link.ID = base.ID;"
        result = df.ida_query(query, autocommit=True)
    ...
    if output_table is None and merge_output is True:
        ...
        query = "create table "+df_output_table+" as select  " + columns_str + " , base.*  from  " + output_table_tmp + "  as link INNER JOIN  " + table_name + " as base on link.ID = base.ID;"
