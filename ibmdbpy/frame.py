...
###############################################################################
    ...
    def delete_na(self, columns, logic = "any", inplace = False):
        ...
        if inplace == True:
            ...
            if answer == 'n':
                print('Then use the default option inplace = False')
                return None
            if answer == 'y':
                print("Any IdaDataFrame pointing this table might be modified accordingly.")
            else:
                print("User input should be either 'y' or 'n'.")
                return None
            ...
            idadb._prepare_and_execute("CREATE TABLE %s LIKE %s"%(tablename,viewname))
            idadb._prepare_and_execute("INSERT INTO %s (SELECT * FROM %s)"%(tablename,viewname))
    ...
    def mapping(self, column_name, dic = None, inplace = True):
        ...
        if not isinstance(column_name, six.string_types):
            raise TypeError("%s is not of string type"%(column_name))
        if column_name not in self.columns:
            raise ValueError("%s refers to a column that doesn't exists in self"%(column_name))
        ...
        if inplace == True: 
            ...
            while new_name in self.columns:
                ...
                c = c+1                
            print('New column name will be: %s' %new_name)
        ...
        self = IdaDataFrame(idadb, self.tablename, indexer = self.indexer)
        idadb.commit()
        print(self.columns)
        try:
            del self.internal_state.columndict
